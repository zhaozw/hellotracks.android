...
public class TrackingSender extends BroadcastReceiver {
	...
	public static final long SEND_INTERVAL = 60000;
	...
	private void sendAsync(final Context context, String username,
			String password, final GPS... locations) {
		try {
				...
				public void run() {
					...
					if (response) {
						...
						int total = settings.getInt(Prefs.LOCATIONS_TOTAL, 0)
								+ track.size();
						settings.edit()
								.putInt(Prefs.LOCATIONS_TOTAL, total)
								.putLong(Prefs.LAST_TRANSMISSION,
										System.currentTimeMillis()).commit();

						if (locations.length >= MAX && ++cycle < 10
								&& !exceptionOcurred) {
							callAgain(context);
						} else {
							cycle = 0;
	...
	private void callAgain(final Context context) {
		...
		alarmManager.setInexactRepeating(AlarmManager.ELAPSED_REALTIME_WAKEUP,
				triggerAtTime, SEND_INTERVAL, sendIntent);
	}
	...
	public static class Scan {
		...
		public static boolean process(JSONObject json) {
			...
			try {
				...
				httppost.setEntity(new ByteArrayEntity(json.toString()
						.getBytes("UTF8")));
				Log.w("-->" + json.toString());
