...
public class TrackingService extends Service {
	...
	public class PowerReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			setCharging(intent.getAction()
					.equals(Intent.ACTION_POWER_CONNECTED));
			reregister();
		}
	}
	...
	public void onCreate() {
		...
		registerReceiver(powerReceiver, new IntentFilter(
	...
	public void startSendManager() {
		...
		alarmManager.setInexactRepeating(AlarmManager.ELAPSED_REALTIME_WAKEUP,
				triggerTime, TrackingSender.SEND_INTERVAL, sendIntent);
	}
	...
	public void startLocationManager() {
		try {
			...
			if (settings.gps) {
				...
				if (true || !wifi.isConnectedOrConnecting()) {
					...
					locationManager.requestLocationUpdates(
							LocationManager.GPS_PROVIDER, settings.minTime,
			...
			if (settings.network) {
				Log.d("starting location manager (minTime=" + settings.minTime
						+ ",minDistance=" + settings.minDistance
						+ ", provider= " + "network" + ")");
				locationManager.requestLocationUpdates(
						LocationManager.NETWORK_PROVIDER, settings.minTime,
						settings.minDistance, locationListener);
			}
		} catch (Exception exc) {
			Log.w(exc);
	...
	private Notification createNotification(boolean trackingOn) {
		...
		boolean gpsmsg = locationManager != null
				&& !locationManager
			...
			if (settings.mode != Mode.fuzzy && gpsmsg) {
				...
				case transport:
					if (charging)
						...
						text = res
								.getString(R.string.trackingWaitingToBeConnected);
						icon = R.drawable.tiny_bw;
					}
					break;
				case fuzzy:
